...
- OpenAPI discovery advertises the production HTTPS base `https://starrocks-mcp.devicesformula.com`, and every path in the specification is namespaced under `/api/...`. Caddy forwards those requests to the local FastAPI instance on port `8002`, so GPT Actions imports work without extra configuration.
