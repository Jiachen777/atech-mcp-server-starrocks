...
- OpenAPI discovery always advertises the production HTTPS endpoint `https://starrocks-mcp.devicesformula.com/api`, which is fronted by Caddy and reverse-proxies requests to the local FastAPI instance on port `8002`. No additional configuration is required for GPT Actions imports.
