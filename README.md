...
- `STARROCKS_DB`: (Optional) Default database to use if not specified in tool arguments or resource URIs. If set, the connection will attempt to `USE` this database. Tools like `table_overview` and `db_overview` will use this if the database part is omitted in their arguments. Leave this unset/empty to allow the API server to switch between every database your StarRocks user can access.
...
- `STARROCKS_API_PUBLIC_URL`: (Optional) The externally reachable base URL (including any reverse-proxy prefix such as `/api`) that should appear in the generated OpenAPI document. Set this to the HTTPS endpoint that GPT Actions or other clients will call. Defaults to `http://localhost:8002` when unset.
- `STARROCKS_FE_ARROW_FLIGHT_SQL_PORT`: (Optional) Enable Arrow Flight SQL transport when your StarRocks cluster exposes the service (generally v3.4+). Leave this unset for StarRocks 3.3 and earlier; the server will automatically fall back to the MySQL protocol.
